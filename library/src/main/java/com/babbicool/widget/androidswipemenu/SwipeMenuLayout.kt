package com.babbicool.widget.androidswipemenu
...
open class SwipeMenuLayout: ViewGroup {
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        val typedArray = context.theme.obtainStyledAttributes(attrs, R.styleable.AndroidSwipeMenu, defStyleAttr, 0)

        try {
            (0 until typedArray.indexCount).forEach { i ->
                when (typedArray.getIndex(i)) {
                    R.styleable.AndroidSwipeMenu_leftView -> {
                        mLeftViewResID = typedArray.getResourceId(R.styleable.AndroidSwipeMenu_leftView, -1)
                    }
                    R.styleable.AndroidSwipeMenu_rightView-> {
                        mRightViewResID = typedArray.getResourceId(R.styleable.AndroidSwipeMenu_rightView, -1)
                    }
                    R.styleable.AndroidSwipeMenu_contentView -> {
                        mContentViewResID = typedArray.getResourceId(R.styleable.AndroidSwipeMenu_contentView, -1)
                    }
                    R.styleable.AndroidSwipeMenu_hasLeftView -> {
                        mCanLeftSwipe = typedArray.getBoolean(R.styleable.AndroidSwipeMenu_hasLeftView, true)
                    }
                    R.styleable.AndroidSwipeMenu_rightView -> {
                        mCanRightSwipe = typedArray.getBoolean(R.styleable.AndroidSwipeMenu_rightView, true)
                    }
                    R.styleable.AndroidSwipeMenu_fraction -> {
                        mFraction = typedArray.getFloat(R.styleable.AndroidSwipeMenu_fraction, 0.5f)
                    }
                }
            }
        } catch (e: Exception) {
